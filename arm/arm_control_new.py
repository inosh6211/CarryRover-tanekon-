...
while True:
    ...
    if pixels is not None:
        ...
        # 物体が見失われた場合
        print("Object lost. Returning to previous position or home position.")
        
        # 物体が見失われた場合にアームを初期位置に戻す（オプション）
        if prev_x_pixel is not None and prev_y_pixel is not None:
            # 徐々にアームを初期位置に戻す
            for step in range(0, 100, 10):
                x_step = 0 * (step / 100)  # 初期位置（0, 0）
                y_step = 0 * (step / 100)
