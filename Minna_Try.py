...
class CameraReceiver:
    ...
    def read_camera(self):
        ...
        while True:
            if self.uart.any():
                print("UARTにデータあり")  # 受信があるかを確認
                char = self.uart.read(1).decode('utf-8')
                #print("読み取った文字:", char)  # 読み取った文字を表示
                if char == "\n":
                    break
                message += char
                time.sleep(0.01)
            else:
                print("UARTにデータなし")  # データがない場合を確認（※頻出する場合はコメントアウトも検討）
                time.sleep(0.1)  # ループの負荷を下げるために少し待つ
                
        #print("最終的なメッセージ:", message)  # 受信した全データを確認    
    ...
    def read_tags(self, mode):
        ...
        data = self.read_camera()
        print("read_camera()が返したデータ:", data)
