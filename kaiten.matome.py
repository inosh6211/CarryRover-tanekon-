    ...
    while True:
        ...
        while True:
            ...
            if detected_id is not None:  
                corrected_distance = 2.5032 * cam.tag_distance[detected_id] - 1.1803
...
def apriltag_guidance(t1, t2):#t1=中心から角#かどからかど
    ...
    if detected_id in [2, 6]:
        ...
        log.sd_write(f"Tag ID {detected_id}: Executing maneuver: right 90°, forward (t1), left 90°, forward (t1), left 90°")
        bno.compute_euler()
        init_yaw = (-bno.yaw + 360) % 360
        ...
        log.sd_write(f"Tag ID {detected_id}: Executing maneuver: right 90°, forward (t1), left 90°, forward (t2), leftt 90°,  forward (t1)")
        bno.compute_euler()
        init_yaw = (-bno.yaw + 360) % 360
        soutai_turn(90, init_yaw)   # 右90°旋回
        straight_ward("f", t1)
        ...
        log.sd_write(f"Tag ID {detected_id}: Executing maneuver: left 90°, forward (t1), right 90°, forward (t1), right 90°")
        bno.compute_euler()
        init_yaw = (-bno.yaw + 360) % 360
        soutai_turn(270, init_yaw)   # 左90°旋回
        straight_ward("f", t1)
