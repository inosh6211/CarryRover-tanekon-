...
while True:
    ...
    if tags:
        for tag_id in range(10):
            for tag in tags:
                if tag_id == tag.id():
                    data_t[6 * tag_id] = tag_id
                    data_t[6 * tag.id() + 1] = 1
                    data_t[6 * tag.id() + 2] = tag.cx() - (ROI_W / 2)
                    data_t[6 * tag.id() + 3] = tag.cy() - (ROI_H / 2)
                    data_t[6 * tag.id() + 4] = tag.z_translation()
                    data_t[6 * tag.id() + 5] = tag.y_rotation()
