...
#pitch分回転させてから90度回転、90度回転させてからpitch分回転どっち？
#2だけじゃなくてすべてのたぐにたいしてにしたい
#init_yawをまたぐとsoutaiが使えない問題
def april_tag_alignment():
    ...
    while True:
        ...
        camera.read_tags(0)
        # 距離補正式
        corrected_distance = 424.115 + (2.5032 * camera.tag_distance[tag_id] - 1.1803)
        # タグのピッチ角（°）を取得
        ...
        if 10 <= ka <= 180:
            ...
            bno.compute_euler()
            init_yaw = (-bno.yaw + 360) % 360
            soutai_turn(-(90 - (-(ka-360))), init_yaw)
