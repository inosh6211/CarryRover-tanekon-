...
class Motor:
    ...
    def update_speed(self, timer):        
        if self.state == 0:
            ...
            # print(rpm_b, self.rate_b, self.target_rpm_b)
            self.run(self.state)
...
class AprilTagNavigator:
    def __init__(self, motor, cam, bno):
        self.motor = motor
        self.cam = cam
        self.bno = bno
        #self.log = log
    
    def apriltag_alignment(self):
        self.motor.enable_irq()
        self.motor.update_rpm(30, 30)
        ...
        while True:
            ...
            # 0～9 のタグIDをすべてチェック
            for i in range(10):  
                if self.cam.tag_detected[i]:
                    detected_id = i
                    break
                      # 最初に見つかったタグを使用
            
            if detected_id is not None:  
                corrected_distance = 424.115 + (2.5032 * self.cam.tag_distance[detected_id] - 1.1803)
                ka = self.cam.tag_pitch[detected_id]
                ...
                self.motor.update_rpm(20, 20)
                self.motor.run(TURN_R)
            
            time.sleep(0.1)
            # タグが見つかった場合はループを抜ける（見つかるまで回り続ける）
            if detected_id is not None:
                break
        
        self.motor.disable_irq()   
    
    def align_to_tag(self, ka, corrected_distance):
        if ka is not None and 10 <= ka <= 180:
            time.sleep(2)
            self.motor.straight_ward("b", 1000, 30)
            time.sleep(2)
            self.motor.stop()
            bno.compute_euler()
            init_yaw = (-bno.yaw + 360) % 360
            self.motor.soutai_turn(ka , init_yaw)
            
            if corrected_distance is not None:
                go_distance = (corrected_distance + 100 )* abs(math.sin(math.radians(ka)))
                self.straight_ward(f, go_distance, 30)
                self.motor.soutai_turn(360 - ka, init_yaw)
            ...
            bno.compute_euler()
            init_yaw = (-bno.yaw + 360) % 360
            self.motor.soutai_turn( 90 - ka , init_yaw)
...
class AprilTagGuidance:
    ...
    def apriltag_guidance(self, index):
        ...
        while True:
            ...
            self.motor.update_rpm(10, 10)
            self.motor.run(TURN_R)
            time.sleep(0.5)
            self.motor.stop()
            time.sleep(0.1)
        
        self.move_to_tag(detected_id, CONES_RADIUS)
    
    def move_to_tag(self, detected_id, radius):
        while True:
            self.cam.read_tags(0)
            if not self.cam.tag_detected[detected_id]:
                #self.log.sd_write("Tag lost. Initiating recovery spin.")
                self.motor.update_rpm(10, 10)
                self.motor.run(TURN_R)
                time.sleep(0.5)
                self.motor.stop()
                continue
            if self.cam.tag_distance[detected_id] <= 20:
                self.motor.stop()
                #self.log.sd_write(f"Goal achieved: Near tag ID {detected_id}.")
                break
            else:
                self.motor.update_rpm(30, 30)
        ...
        self.perform_maneuver(detected_id, radius)
    
    def perform_maneuver(self, detected_id, radius):
        maneuver_map = {
            4: [90, radius + 20, -90, radius + 20, -90],   #radiusに足されている数は地上局の半径
            8: [90, radius + 20, -90, radius + 20, -90],
            3: [-90, radius + 20, 90, radius + 20, 90],
            7: [-90, radius + 20, 90, radius + 20, 90],
            5: [90, 2 * radius + 20, -90, radius + 20, -90],
            9: [90, 2 * radius + 20, -90, radius + 20, -90],
        }
        if detected_id in maneuver_map:
            steps = maneuver_map[detected_id]
            for step in steps:
                if isinstance(step, int):
                    self.bno.compute_euler()
                    init_yaw = (-self.bno.yaw + 360) % 360
                    self.motor.soutai_turn(step, init_yaw)
                else:
                    self.motor.straight_forward_t(step, 30)
        self.motor.disable_irq()
...
if __name__ == "__main__":
    ...
    navigator = AprilTagNavigator(motor, cam, bno)
    guidance = AprilTagGuidance(motor, cam, bno)
    ...
    try:
        ...
        color_guidance(0)
        """
        navigator.apriltag_alignment()
        guidance.apriltag_guidance(0)
        """
        collect_material(0)
        gps_guidance(1)
        color_guidance(1)
